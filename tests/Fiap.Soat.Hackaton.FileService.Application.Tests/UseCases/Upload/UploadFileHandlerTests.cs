...
namespace Fiap.Soat.Hackaton.FileService.Application.Tests.UseCases.Upload;
...
public class UploadFileHandlerTests
{
    ...
    public async Task Handle_ShouldUploadPersistPublishAndReturnSuccess_WhenPdfIsValid()
    {
        ...
        storageMock
            .Setup(x => x.UploadFileAsync(It.IsAny<Fiap.Soat.Hackaton.FileService.Domain.Entities.FileDocument>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(ResponseFactory.Ok(storageDetails));

        repositoryMock
            .Setup(x => x.InsertAsync(It.IsAny<Fiap.Soat.Hackaton.FileService.Domain.Entities.FileDocument>(), It.IsAny<CancellationToken>()))
